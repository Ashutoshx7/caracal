...
//
// View factories for resource lists and detail panels backed by AdminClient.
...
export function zonesView(ctx: Ctx): View {
  return new ListView<Zone>({
    ...
    onEnter: (app, row) => app.push(detailViewFor(`zone / ${row.slug}`, () => ctx.client.zones.get(row.id))),
  })
}
