...
The coordinator derives `agent_kind` from the application: a session under a managed application is `instance`, and a session under a DCR application is `ephemeral`. Because `spawn()` always runs under the caller's own application, a spawned child of a managed application is `instance` — through the SDK, `spawn()` does not produce an `ephemeral` session. An `ephemeral` agent exists as the single session of a DCR application.

`instance` and `ephemeral` are **descriptors, not runtime behaviors**: they carry the same heartbeat rules, the same TTL sweeping, and the same enforcement. The one `agent_kind` with distinct runtime behavior is `service`, which holds a heartbeat lease. The `instance`/`ephemeral` split simply mirrors `registration_method` (managed vs DCR) so policy and audit can name the credential boundary; it does not change what an agent may do or how long it lives.

A **short-lived worker is not a separate kind** — it is an ordinary `instance` session created with a TTL. The mainstream tree of a long-lived orchestrator, mid-tier managers, and disposable task workers is modelled under **one managed application**:
...
Every node in that tree is an `instance` session under the one managed app, distinguished by its `agent_session_id`, `labels`, and delegation context. "Disposable worker" is expressed by the TTL and the narrowed grant, not by an `ephemeral` kind.
