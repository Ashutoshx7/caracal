...
Yes. The agent's identity in a decision is the agent session, not the application. Each token exchange carries the acting `agent_session_id`, `agent_kind`, and `capabilities`, plus the delegation edge and chain, alongside the `application_id`. Policy reads these as `input.principal.agent_session_id`, `input.principal.agent_kind`, and `input.principal.capabilities`, and audit metadata records the same fields with the parent and delegation context. Two agents under one managed application present different sessions, capabilities, and delegation paths, so they are fully distinguishable.

A single managed application is therefore the right fit for one durable workload that runs many agents. Separate per-agent identity comes from spawning an agent session per agent with role-specific capabilities, not from creating an application per agent. Reach for a DCR application per agent only when a spawned agent needs an isolated, expiring credential of its own. What you gain from separate applications is credential isolation, not policy or audit distinguishability.
