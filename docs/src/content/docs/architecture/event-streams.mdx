...
The `_sig` field is excluded from its own signature. Consumers verify the signature before acting on any message. Services that run with `CARACAL_MODE=runtime` require `STREAMS_HMAC_KEY` on stream paths they publish or verify; Gateway requires it at startup. Development-mode publishers and consumers may run without a stream key and log that origin verification is disabled.
