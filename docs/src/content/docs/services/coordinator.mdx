...
`dead` is the Console state for invocations that exhausted all retry attempts without a successful result.
...
The Coordinator does not run database migrations. Schema changes are applied before service startup through the Compose `dbMigrate` service or the Helm migration Job.
...
| `STS_URL` | None | STS URL for agent token exchanges |
| `ISSUER_URL` | None | JWT issuer URL for JWKS verification (also used as the required `aud`) |
| `AGENT_COORDINATOR_SCOPE` | None | Required scope in bearer JWTs |
