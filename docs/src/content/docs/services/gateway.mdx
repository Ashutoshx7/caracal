...
`CARACAL_MODE=runtime` enables startup safety checks. It rejects `INSECURE_HTTP`, `INSECURE_STS`, and `JTI_FAIL_OPEN`; requires `REDIS_URL`, `STREAMS_HMAC_KEY`, and `AUDIT_HMAC_KEY`; requires port `8081`; and requires `UPSTREAM_HOST_ALLOWLIST` when private upstreams are enabled. `STS_URL` must be absolute. Runtime mode allows an HTTP STS URL only for internal hosts such as `sts`, `localhost`, loopback, or another single-label service name.
...
| `STS_URL` | — | **Required.** STS base URL with scheme. In runtime mode, HTTP is allowed only for internal hosts. |
...
| `AUDIT_HMAC_KEY` | — | Required when `CARACAL_MODE=runtime`; verifies audit-linked stream events |
| `TLS_CERT_FILE` | — | TLS certificate; must be set with `TLS_KEY_FILE` |
| `TLS_KEY_FILE` | — | TLS private key; must be set with `TLS_CERT_FILE` |
| `INSECURE_HTTP` | `false` | Developer-only flag; forbidden when `CARACAL_MODE=runtime` |
| `INSECURE_STS` | `false` | Developer-only flag; forbidden when `CARACAL_MODE=runtime` |
...
| `JTI_FAIL_OPEN` | `false` | Allow requests if JTI Redis lookup fails; forbidden when `CARACAL_MODE=runtime` |
