...
When `CARACAL_MODE=runtime`, the control service also requires `CONTROL_REDIS_URL`, `AUDIT_HMAC_KEY`, and at least one registered upstream implementation. The current reference compose profile is intended for development validation of the surface, not as a complete production control-plane deployment.
