...
```bash
the terminal management interface list --zone <zone-id> --subject <user-id> --status active
```

Or query the audit log:

```bash
the terminal management interface tail --zone <zone-id> --limit 100 | grep "<subject>"
```

**Step 2 — Revoke the session:**

```bash
the terminal management interface revoke --zone <zone-id> --session-id <sid>
```
...
```bash
curl http://localhost:8081/metrics | grep -E 'caracal_gateway_(revocations_active|denials_revoked_total)'
```

`caracal_gateway_revocations_active` should reflect the revoked session. `caracal_gateway_denials_revoked_total` should increment if the session's mandates are still being presented.
...
**Step 4 — Replace affected zone signing material through a controlled operation:**

Self-service zone signing-key rotation is not packaged as a terminal management interface/API operation in the OSS control plane. If signing material may have been exposed, perform the database-backed signing-key replacement through your deployment automation or incident runbook, then invalidate STS key caches and verify JWKS for every affected zone before restoring traffic.
...
the terminal management interface list --zone <zone-id> --status active | xargs -n1 the terminal management interface revoke --zone <zone-id> --session-id
```
...
```bash
the terminal management interface tail --zone <zone-id> --since <ISO-8601-start> --until <ISO-8601-end> --event-type token_issued
```
...
**Detection:** `caracal_audit_tamper_mismatch_total > 0` or `caracal_audit_tamper_chain_breaks_total > 0` in Audit `/metrics`, or an entry in `audit_ingest_alerts`.
...
```bash
curl http://localhost:8080/metrics | grep -E 'caracal_sts_audit_(dropped_total|replay_pending)'
```

A non-zero `caracal_sts_audit_dropped_total` value means audit events were lost during the outage. Document the outage window for audit record purposes.

---

## Playbook: Audit DLQ growing

**Detection:** `caracal_audit_dlq_total` increasing in Audit `/metrics`, or `XLEN caracal.audit.events.dlq > 0`.
