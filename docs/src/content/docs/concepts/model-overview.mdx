...
You configure these directly. A *grant* — a permission binding for resource scopes — is an internal input that policy can read during evaluation, not a separate object you manage in the Console.
...
Next, read [Authority Model](/concepts/authority-model/) to see where each enforcement layer fits.
