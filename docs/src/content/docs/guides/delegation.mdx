...
description: Open child agent sessions, attach typed constraints, traverse the delegation graph, and trigger cascade revocation using the SDK and terminal interface.
pageType: workflow
---
...
| `max_hops` | Caps delegation graph traversal hops |
| `ttl_seconds` | Caps the delegated authority lifetime |
| `budget` | Carries a numeric budget constraint for policy evaluation |
| `policy_approved` | Marks policy-reviewed delegation intent |
| `expires_at` | Hard expiry for the edge independent of the session TTL |
| `broad_reason` | Records the reason for a broad delegation |

The STS validates all constraints on each exchange. A constraint violation fails the exchange before OPA is evaluated.

## Read the delegation graph from the terminal interface
