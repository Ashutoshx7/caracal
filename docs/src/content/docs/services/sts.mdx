...
```json
{
  ...
  "challenge_type": "mfa",
  "challenge_secret": "<single-use proof>",
...
**Evaluation:** The compiled bundle evaluates the `data.caracal.authz` package. The input is a structured object with six top-level keys:

- `input.principal`: `{type, id, zone_id, credential_type, agent_session_id}`
- `input.resource`: `{type, id, identifier, scopes}` (one call per resource)
- `input.action`: `{id}` (always `"TokenExchange"`)
- `input.session`: `{id}` (omitted when no session context)
- `input.delegation_edge`: `{id, source_session_id, target_session_id, issuer_application_id, receiver_application_id, resource_id, scopes, edge_version, path, graph_epoch, constraints_json}` (omitted when no delegation)
- `input.context`: `{actor_claims, subject_claims, trace_id, session_id, agent_session_id, delegation_edge_id, challenge_resolved, requested_scopes}`

**Decision:** Policies must emit a `result` object with `decision`, `evaluation_status`, `determining_policies`, and `diagnostics` fields. If the result's `decision` is `deny`, the exchange is rejected with 403.
...
| `caracal.keys.invalidate` | Flush the zone key cache on signing key rotation |

**Keys:**

- `audit:jti:{jti}`: JTI deduplication set. TTL matches token lifetime. Prevents replay attacks.
...
7. Replay any pending events from `AUDIT_REPLAY_DIR` (re-publishes disk-buffered events to Redis).
8. Start audit buffer flush goroutine: drains the in-process AuditBuffer to `caracal.audit.events`.
9. Start Redis stream consumers: `caracal.sessions.revoke` (session revocation), `caracal.policy.invalidate` (OPA recompile), `caracal.keys.invalidate` (key cache invalidation).
10. Start OPA polling goroutine: polls for policy changes every `OPA_POLL_SECONDS`.
11. Start OPA seed goroutine: preloads all zone policies from PostgreSQL at startup.
12. Listen on `0.0.0.0:8080`.
...
| `ISSUER_URL` | None | JWT `iss` claim; must match the STS base URL |
| `ZONE_KEK` | None | 32-byte hex zone encryption key (required) |
