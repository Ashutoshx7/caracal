...
Create a local fixture with the same schema shape when you need a repeatable simulation input:
...
cat > input.json <<'JSON'
{
  ...
  "context": {
    "requested_scopes": ["payment:read", "payment:submit"]
  }
}
JSON
```
...
The terminal interface exposes the same check:
