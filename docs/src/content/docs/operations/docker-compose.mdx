...
```bash
caracal-cli control mount
caracal-cli control enable
caracal-cli control status
caracal-cli control disable
caracal-cli control unmount
```
...
`CARACAL_CONTROL_ENABLED` is injected by the engine lifecycle path; do not set it directly or start the profile manually.
