...
- **Status vocabulary.**
  ...
  - **Not covered** — Caracal does not address this item; it is the adopter's or another system's responsibility.
  - **Not applicable** — The item does not map to Caracal's function.
...
Adoption recommendation in one line: **suitable as a least-privilege authority, revocation, and audit-evidence control for agentic systems, on the explicit understanding that the adopter owns the surrounding compliance program and the items marked partial or not covered below.**
...
| --- | --- | --- | --- |
| ASI01 | Memory poisoning | Not covered | Agent memory/state is outside Caracal. Mitigate in the application. |
...
| ASI05 | Cascading hallucination | Not covered | Model-output correctness is outside Caracal's function. |
...
| Map | Not covered (supporting) | Caracal does not perform risk/context mapping. Its trust-boundary and resource model are inputs an adopter can map against. |
...
| --- | --- | --- |
| Art. 9 — Risk management system | Not covered (supporting) | Provides enforcement and revocation controls an adopter can reference; the risk-management system is the adopter's. |
...
| --- | --- | --- |
| CC6 — Logical & physical access | Covered (technical control) | Sole-issuer STS, signed least-privilege mandates, per-zone RLS, service DB roles with `NOBYPASSRLS`, centralized revocation. |
...
| P-series — Privacy | Not covered | Caracal does not implement data-subject-rights workflows; it processes authority metadata, not subject data sets. |
...
| Model-behavior threats (ASI01/05/06/07) | Out of scope (by design) | Caracal governs authority and records actions; it does not evaluate model reasoning or output. |
