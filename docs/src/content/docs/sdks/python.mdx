...
Reads all `CARACAL_*` environment variables and constructs a `Caracal` instance. Static-token mode uses `CARACAL_SUBJECT_TOKEN`. Client-secret mode uses `CARACAL_APP_terminal interfaceENT_SECRET`, `CARACAL_STS_URL`, and resources from `CARACAL_APP_RESOURCES`, `CARACAL_RESOURCES`, or `CARACAL_RESOURCES_FILE`.
...
Builds a client from `caracal.toml`. If `path` is omitted, the SDK reads `CARACAL_CONFIG` or falls back to `${XDG_CONFIG_HOME:-~/.config}/caracal/caracal.toml`. The config must provide `zone_id`, `application_id`, `app_client_secret`, an STS URL (`sts_url`, `zone_url`, or `CARACAL_STS_URL`), a Coordinator URL (`coordinator_url` or `CARACAL_COORDINATOR_URL`), and at least one resource binding.
