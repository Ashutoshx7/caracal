...
To remove the deployment, `caracal-deploy/containerPlatform/scripts/destroyAzure.sh` deletes every Caracal app and job it created and leaves the database, cache, vault, registry, and environment in place, so audit evidence and credentials survive the teardown.
