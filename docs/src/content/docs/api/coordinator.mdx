...
| --- | --- | --- | --- |
| `application_id` | string | Yes | None |
...
| `ttl_seconds` | integer | No | `3600` | 1 to 86400 |
| `metadata` | object | No | `{}` |
...
**Query parameters:** `limit` (1 to 500, default 100), `cursor` (agent ID)
...
**Query parameters:** `reason` (string, 1 to 256 characters; default `"requested"`)
...
| `ttl_seconds` | integer | Cond.; 1 to 86400; alternative to `expires_at` |
...
| `max_hops` | integer | Maximum delegation chain depth, 1 to 10; default 1 |
| `max_depth` | integer | Compatibility alias for `max_hops`, 1 to 10 |
...
| `invalid_max_hops` | 400 | `max_hops` is outside the supported 1 to 10 range |
| `invalid_max_depth` | 400 | `max_depth` is outside the supported 1 to 10 range |
...
**Query parameters:** `limit` (1 to 500, default 100), `cursor` (edge ID)
...
| `GET` | `/metrics` | Prometheus text counters for agent lifecycle and delegation operations |
| `GET` | `/stats` | JSON operational snapshot for scripts |
