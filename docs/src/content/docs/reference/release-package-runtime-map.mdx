...
`release.config.json` `product.version` is authoritative. The current source state is `1.0.0-rc.1`; successive release candidates increment `-rc.N`.
...
Use [Wire Contracts](/reference/interoperability-contracts/) when validating SDK, adapter, exporter, or connector compatibility.
