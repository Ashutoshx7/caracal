...
## Scope
- Covers the `caracalai-asgi` Python package under `packages/connectors/asgi/python/`.

## Architecture Design
- The package adapts `caracalai_transport_mcp` verification to the ASGI protocol.
- It is framework-neutral: it must run under FastAPI, Starlette, Quart, or any ASGI server without importing any of them.

## Required
- Must call `caracalai_transport_mcp` (`MandateVerifier`/`authenticate`) for token verification.
- Must require a caller-provided revocation store and forward it to transport authentication.
