...
- **The gateway proxy client follows HTTP redirects; the STS provider client does not.**
  - *The limit.* `services/gateway/internal/proxy.go` builds its upstream `http.Client` without a `CheckRedirect`, so it follows up to Go's default of ten 3xx redirects. Because the per-request upstream carries an injected credential — a provider OAuth/API-key value in a possibly operator-named custom header, `X-Caracal-Identity`, or the STS `Authorization` bearer — a compromised, misconfigured, or (over plain `http`) on-path upstream can redirect the gateway to a host it chooses. Go strips `Authorization` on a cross-registrable-domain hop but preserves custom headers, so a custom-header provider credential and `X-Caracal-Identity` can be forwarded to the redirect target, and the `AllowedTokenHosts`/egress-allowlist checks — evaluated only against the initial upstream — do not re-apply to redirect targets, so a redirect also amplifies egress beyond the allowlist.
  - *How Caracal contains it.* The connection-time `SafeDialContext` re-resolves every hop (including redirect targets) and still refuses link-local/cloud-metadata, multicast, unspecified, and NAT64-embedded dangerous addresses, so a redirect cannot reach the metadata service or other always-blocked infrastructure. Upstreams and their credentials are operator-provisioned, so the exposure is bounded to the credentials an operator already entrusted to a given upstream. The correct pattern is already implemented in `services/sts/internal/refresh.go` (`safeHTTPClient` returns `http.ErrUseLastResponse`).
  - *Path to closure.* Set `CheckRedirect` on the gateway proxy client to return `http.ErrUseLastResponse` (surfacing the upstream 3xx to the caller unfollowed), matching the STS provider client, and add a gateway test asserting redirects are not followed.
