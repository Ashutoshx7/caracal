...
HMAC-signs Redis stream messages for session revocation, policy invalidation, audit, and lifecycle events. Required when `CARACAL_MODE=runtime` for services that publish or verify signed streams; Gateway requires it in all modes.
