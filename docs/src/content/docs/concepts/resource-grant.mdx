...
The Gateway reads the `upstreams` directive and substitutes the provider credential as the Authorization header sent to the upstream. Normal STS clients do not receive the provider-native credential. The agent only presents its mandate to the Gateway, and the Gateway handles provider authentication.
...
2. Looks up the resource binding in its binding store (populated from `gateway_resource_bindings`) to find the zone and Gateway application identity for the resource.
