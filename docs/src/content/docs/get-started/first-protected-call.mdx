...
| Access policy | Let the web console create and activate the starter policy. |
...
Guided setup can write the runtime profile and secret file for you. Enable **write profile files** when you want the web console to create them on the current machine.

If file writing is disabled, copy the generated TOML profile and one-time app client secret from the result view into the paths the web console shows. Keep the secret file readable only by the owner.
...
Run the exact `curl` or PowerShell command from the web console result view first. It uses the resource ID, protected path, and profile values you just created.
...
3. Open the event detail, or open **Request Traces** and paste its request ID.
