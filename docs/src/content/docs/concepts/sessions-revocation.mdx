...
When an agent is terminated, the Coordinator publishes both the STS session ID and the agent session ID. The STS persists the STS session revocation, and the Gateway tracks both IDs so already-issued mandates are blocked even though their signatures remain valid.
