...
| `AUDIT_HMAC_KEY` | STS, Audit, Gateway, Control | 32-byte hex-encoded key for audit event chain HMAC. Required when `CARACAL_MODE=runtime` on services that verify or emit protected audit events. |
| `STREAMS_HMAC_KEY` | STS, API, Gateway, Coordinator, coordinator relay | 32-byte hex-encoded key for signing or verifying Redis stream messages. Required by Gateway; required by STS, API, Coordinator, and relay when `CARACAL_MODE=runtime`. |
...
| --- | --- | --- |
| `CARACAL_MODE` | `runtime` in service code; `dev` in local Docker Compose | `runtime` or `dev`. Runtime mode enables startup safety checks and requires signed stream/audit keys on protected paths. |
...
| `AUDIT_HMAC_KEY` | — | Required when `CARACAL_MODE=runtime`. 32-byte hex-encoded HMAC key |
...
| `STREAMS_HMAC_KEY` | — | Required when `CARACAL_MODE=runtime`. 32-byte hex-encoded key for signing Redis stream messages |
...
| `HOST` | `0.0.0.0` in runtime, `127.0.0.1` otherwise | Listen host |
| `WORKER_ID` | Derived from hostname and process ID | Outbox worker identifier |
| `STREAMS_HMAC_KEY` | — | Required when `CARACAL_MODE=runtime`. 32-byte hex-encoded key for signing outbox stream messages |
...
| `CARACAL_MODE` | `runtime` | `runtime` or `dev`; controls runtime safety checks |
| `STREAMS_HMAC_KEY` | — | **Required.** 32-byte hex-encoded key for verifying stream signatures |
| `AUDIT_HMAC_KEY` | — | Required when `CARACAL_MODE=runtime` |
...
| `UPSTREAM_HOST_ALLOWLIST` | — | Comma-separated upstream host allowlist. Required when `CARACAL_MODE=runtime` and `ALLOW_PRIVATE_UPSTREAMS=true`. |
| `JTI_FAIL_OPEN` | `false` | Skip JTI replay check on Redis error. Forbidden when `CARACAL_MODE=runtime`. |
...
| `DATABASE_URL` | — | **Required.** Postgres connection string |
| `REDIS_URL` | — | **Required.** Redis connection string |
| `AUDIT_HMAC_KEY` | — | Required when `CARACAL_MODE=runtime`. 32-byte hex-encoded HMAC key for audit chain verification |
| `PORT` | — | **Must be `9090`.** |
...
The control service is pre-release and disabled by default. In Docker Compose it is available only through the `control` profile.
...
| `CONTROL_REDIS_URL` | — | Replay cache and audit sink Redis URL. Required when `CARACAL_MODE=runtime`; optional in dev mode. |
| `AUDIT_HMAC_KEY` | — | Required for control audit events when `CARACAL_MODE=runtime` |
