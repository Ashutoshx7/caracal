...
`@caracalai/transport-a2a` executes delegated calls between agents over HTTP. It performs a two-step operation: exchange the caller's session mandate for a resource mandate at the STS, then POST the call to the target agent's `/a2a` endpoint with that mandate and the full Caracal context envelope.
...
- `InteractionRequiredError` — STS token exchange requires step-up; contains `challengeId`. The transport package does not carry `challenge_secret` or submit `challenge_response`; use the direct STS token endpoint for flows that must complete challenge consumption.
...
async function callPaymentAgent(
  ...
  const response = await a2aCall(
    ...
    process.env.CARACAL_APPLICATION_ID!,
    {
      ...
      clientSecret: process.env.CARACAL_APP_terminal interfaceENT_SECRET!,
    },
  );
...
async function callWithStepUp(subjectToken: string) {
  try {
    ...
    if (err instanceof InteractionRequiredError) {
      ...
      throw new Error('Retry the exchange through the STS token endpoint with challenge_id and challenge_response.');
    }
    throw err;
  }
}
