...
Environment config is considered when workload identity or credential-manifest variables are present, including `CARACAL_APPLICATION_ID`, `CARACAL_APP_CLIENT_SECRET`, `CARACAL_APP_CLIENT_SECRET_FILE`, `CARACAL_RUN_CREDENTIALS`, or `CARACAL_RUN_CREDENTIALS_FILE`.
