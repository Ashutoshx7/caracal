...
To make agents distinguishable in policy and audit, pass `kind` and `capabilities`. These become `input.principal.agent_kind` and `input.principal.capabilities`, so several agents under one application stay separable without one application per agent. `capabilities` are descriptive labels for policy and audit, not grants; authority always comes from scopes and delegation.

```ts
await caracal.spawn(async () => {
  const headers = await caracal.headersAsync();
  await fetch("https://api.example.com/tickets", { headers });
}, { kind: "instance", capabilities: ["refund-agent"] });
```
