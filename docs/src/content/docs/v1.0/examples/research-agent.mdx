...
pageType: workflow
slug: v1.0/examples/research-agent
---
...
Use it to evaluate one-shot runtime credential injection for an existing CLI that cannot accept an SDK transport. The launcher limits described in [Run an Agent with caracal run](/v1.0/guides/runtime-run/) apply: do not use it for a long-running service or as proof of request-level Gateway enforcement.
...
Continue to [Run Lynx Capital](/v1.0/examples/lynx-capital/) when you want a full app reference lab with live Console inspection.
