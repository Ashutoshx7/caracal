...
| `provider` | Gateway upstream auth modes for Caracal mandates, OAuth2 authorization code, OAuth2 client credentials, API-key, and bearer-token upstreams. |
| `resource` | Protected resource identifiers, scopes, upstream URLs, Gateway bindings, and provider links. |
