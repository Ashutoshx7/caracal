...
DCR is for identity isolation, not for ordinary spawn fan-out. The common case — a runtime spawning many child agents — uses **one managed application**: every child is an `instance` agent session under that managed app, distinguished by its `agent_session_id`, `labels`, and delegation context. Reach for DCR only when a child needs its own separate, auto-expiring credential boundary (for example a per-tenant identity). Registering that DCR application is a control-plane operation through the zone DCR endpoint; the SDK `spawn()` primitive never registers applications and always runs children under the caller's existing application. A DCR application carries exactly one ephemeral agent session and, because it binds a single session, cannot itself be the parent that spawns additional sessions.
