...
const ALLOWED_ALGS = new Set(['RS256', 'RS384', 'ES256'])
...
export async function validateJwt(
  ...
  if (!header.alg || !ALLOWED_ALGS.has(header.alg)) {
    throw new Error(`Unsupported or disallowed algorithm: ${header.alg ?? 'none'}`)
  }
  ...
  if (key.alg && key.alg !== header.alg) throw new Error('JWT alg does not match JWK alg')
...
function algorithmFor(alg: string): AlgorithmIdentifier | EcdsaParams {
  if (alg === 'RS256' || alg === 'RS384') return { name: 'RSASSA-PKCS1-v1_5' }
...
async function importJwk(key: JwksKey, alg: string): Promise<CryptoKey> {
  ...
  if (alg === 'RS384') {
    return crypto.subtle.importKey(
      'jwk', key as JsonWebKey,
      { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-384' }, false, ['verify'],
    )
  }
  return crypto.subtle.importKey(
    'jwk', key as JsonWebKey,
