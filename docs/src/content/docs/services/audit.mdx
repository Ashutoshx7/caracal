...
The Audit service exposes `/metrics` as Prometheus text for operator probes and `/metrics.json` as a compatibility JSON snapshot. Important metrics:

| Metric | Meaning |
| --- | --- |
| `caracal_audit_consumer_lag` | Number of pending entries in the audit consumer group |
| `caracal_audit_consumer_pel_oldest_seconds` | Age of the oldest unacknowledged pending entry |
| `caracal_audit_dlq_total` | Cumulative events moved to the DLQ |
| `caracal_audit_dlq_size` | Current Redis DLQ stream length |
| `caracal_audit_dlq_oldest_age_seconds` | Age of the oldest DLQ entry |
| `caracal_audit_tamper_mismatch_total` | Content hash mismatches found by tamper sweeps |
| `caracal_audit_tamper_chain_breaks_total` | Broken hash-chain links found by tamper sweeps |
| `caracal_audit_tamper_hmac_failures_total` | Chain HMAC verification failures |
| `caracal_audit_tamper_last_sweep_unix` | Last rolling tamper sweep timestamp |
| `caracal_audit_tamper_last_full_unix` | Last full retention-window tamper sweep timestamp |
| `caracal_audit_export_errors_total` | S3/Parquet export failures |
