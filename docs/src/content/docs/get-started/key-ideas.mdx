...
Most users should not operate Caracal by thinking about every internal object. The practical workflow is:

1. Register the agent application that will act.
2. Register the resources, tools, or providers the agent may use.
3. Attach a policy that says when access is allowed.
4. Run the agent through Caracal's SDK, CLI, Gateway, or connector.
5. Inspect the audit trail to see what was allowed, denied, proxied, or revoked.

The lower-level concepts on this page explain how Caracal keeps that workflow safe. Zones isolate tenants, applications identify workloads, resources describe targets, mandates carry short-lived authority, delegation links agent sessions, and audit records what happened. End users should usually see those as configuration and observability details, not as separate operational steps.
...
Gateway-proxied resources use an exchange-per-request flow: the Gateway verifies the inbound mandate, exchanges it with STS for the bound resource, substitutes any brokered provider credential, forwards the upstream request, and audits the execution result. SDKs may cache STS exchange results for the same session, resource, delegation, and scope context, but the Gateway still enforces replay and revocation at request time.
...
Gateway-proxied external calls also emit action audit events after STS authorization. Those events record whether request construction failed, upstream transport failed, or the upstream returned a status code; they do not store provider credentials or response bodies.
...
Provision them using the CLI admin commands (`caracal zone create`, `caracal app create`, `caracal resource create`, `caracal policy create`, `caracal policy-set activate`, `caracal grant create`) or the `@caracalai/admin` SDK. The same flow applies in development and production.
...
Use `CARACAL_MODE=stable` for stable deployments, `CARACAL_MODE=rc` for rc deployments, and `CARACAL_MODE=dev` for development. rc and stable reject developer-only escape hatches such as insecure STS or Gateway flags; configure Gateway TLS directly with certificate files or terminate TLS at a trusted ingress.
