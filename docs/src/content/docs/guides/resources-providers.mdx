...
| `apikey` | Forwards an encrypted provider grant value as an API-key credential using `header_name` or `auth_header` from provider config. Caracal does not mint API keys; the key must already exist in the provider-grant storage path. |
...
If no active provider grant exists for the configured resource/provider pair, STS denies the Gateway exchange instead of falling back to a different provider or exposing provider credentials to the caller.
