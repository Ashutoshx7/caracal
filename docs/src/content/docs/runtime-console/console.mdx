...
`caracal-console` is the Console for inspecting and managing Caracal entities without leaving the shell. It fetches from the control-plane API and renders into a full-screen terminal buffer with keyboard navigation.
...
On startup, the Console enters full-screen alt buffer mode and hides the cursor. Press `q` or `ctrl-c` at any time to exit and restore the terminal.
