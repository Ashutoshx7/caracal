...
| Field | Meaning |
| --- | --- |
| `zone_url` | STS zone URL for token exchange. |
| `sts_url` | SDK-readable STS alias/fallback. |
| `coordinator_url` | SDK/Console Coordinator URL. |
| `gateway_url` | Gateway URL for SDK transports. |
...
Credential entries use `env`, `resource`, and optional `upstream_prefix`.
