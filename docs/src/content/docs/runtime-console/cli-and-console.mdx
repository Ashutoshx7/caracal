...
Use the Control API or Admin SDK when scripts need to create zones, applications, resources, policies, grants, authority sessions, agents, delegation edges, audit queries, or Control credentials. Enable Control from the Console `control` menu, then call the authenticated HTTP surface from CI or automation.
