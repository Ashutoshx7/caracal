...
After you create the provider, bind it to the resource, and activate a policy, run the [Provider Preflight](/examples/) (`examples/providerPreflight`). It checks the resource-to-provider binding, OAuth token endpoint host, callback reachability, upstream reachability, runtime-injection eligibility, and that the active policy set actually returns `allow` for your application, resource, and scopes.
