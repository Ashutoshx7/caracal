...
To make agents distinguishable in policy and audit, pass `kind` and `capabilities` when you spawn. These become `input.principal.agent_kind` and `input.principal.capabilities`, so several agents under one application stay separable without one application per agent. `capabilities` are descriptive labels for policy and audit, not grants; authority always comes from scopes and delegation.

```python
async with caracal.spawn(kind="instance", capabilities=["refund-agent"]):
    headers = caracal.headers()
```
