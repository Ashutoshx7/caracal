...
function drawFrames() {
  ...
  drawZone(
    ...
    'Agent hierarchy, delegation scopes, and execution state.',
    'rgba(30, 91, 216, 0.12)',
  )
  drawZone(
    RUNTIME_X - 20,
    ...
    frameH,
    'Service runtime',
    'Resilience, retries, telemetry, audit.',
    'rgba(93, 70, 163, 0.16)',
  )
  drawZone(
    SERVICE_X - 20,
    ...
    frameH,
    'External systems',
    'Providers, ledgers, banks, tax, and compliance services.',
    'rgba(13, 110, 114, 0.14)',
  )
}
...
function placeInternalNodes(stageLayouts) {
  for (const layout of Object.values(stageLayouts)) {
...
function drawInternalEdges() {
  for (const node of Object.values(nodes)) {
    ...
    const color = selected === `node:${node.id}` || selected === `node:${parent.id}` ? 'rgba(30, 91, 216, 0.58)' : 'rgba(30, 91, 216, 0.22)'
