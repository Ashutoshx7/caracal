---
title: FAQ
description: Stable answers to recurring Caracal modeling, runtime, provider, troubleshooting, and edition questions.
pageType: reference
---
...
| Model | [Should I create one application per agent?](#faq-one-application-per-agent) |
| Model | [When should I use a managed application versus DCR?](#faq-managed-application-versus-dcr) |
| Model | [If many agents share one managed application, can policy and audit still tell them apart?](#faq-distinguish-agents-under-one-application) |
...
| Editions | [Which features are Enterprise Edition only?](#faq-enterprise-only-features) |
...
See [Authority and Enforcement](/concepts/authority-model/) and the [Caracal Mental Model](/concepts/model-overview/).
...
To serve many of your own customers from one deployment without per-customer zones, see [Serve Your Own Customers](/guides/serve-customers/).

See [Compare Editions](/enterprise/) for the commercial feature boundary.
...
<a id="faq-one-application-per-agent"></a>
...
The default model is **one managed application per durable workload, with many agent sessions under it**. When that workload fans out — spawning sub-agents, delegating a narrower slice of authority, or building an agent hierarchy for a single request — each of those is a new agent session under the *same* application, not a new application. Policy and audit still tell them apart by agent session, kind, labels, and delegation chain (see [the next answer](#faq-distinguish-agents-under-one-application)).

One application per agent breaks this. Managed applications are operator-provisioned, so a workload cannot mint one per spawned agent at runtime. DCR can register at runtime, but a DCR application is meant to expire and bind exactly one ephemeral session, so per-request fan-out would register and tear down an application for every sub-agent — adding registration latency, credential sprawl, and registry churn for no policy or audit benefit. Reach for an application per agent only when a spawned agent genuinely needs its own isolated, expiring credential and a registry-visible identity; then use a DCR application with one ephemeral session, as described in [managed versus DCR](#faq-managed-application-versus-dcr).

See [Identities and Applications](/concepts/principal/) and the [Caracal Mental Model](/concepts/model-overview/).
...
See [Identities and Applications](/concepts/principal/).

<a id="faq-distinguish-agents-under-one-application"></a>
...
The agent's identity in a decision is the agent session, not the application. You do not pre-register agents in the Console; the workload that holds the application credential creates each agent session at runtime and declares its `agent_kind` and `labels` then. The token exchange carries the acting `agent_session_id`, `agent_kind`, and `labels`, plus the delegation edge and chain, alongside the `application_id`. Policy reads these as `input.principal.agent_session_id`, `input.principal.agent_kind`, and `input.principal.labels`, and audit records the same fields with the parent and delegation context. The Console agent list shows the `kind` and `labels` of every session, so you can see which agent is which under a shared application. So two agents under one managed application present different sessions, kinds, labels, and delegation paths, and both policy and audit can tell them apart without referencing the application.

What is actually enforced is the **authority**, not the label. Spawning is gated by application ownership, the asserted `agent_session_id` is bound to the owning application at exchange, and requested scopes must stay within the delegation chain, which is itself bounded by the parent's authority. The `agent_kind` and `labels`, by contrast, are asserted by the credentialed workload and are not delegation-constrained. They are a sound basis for policy and audit when you control and trust the spawner, but they are not a defense against a compromised workload mislabeling its own sub-agents. For that, the boundary is scopes, delegation, and policy.
...
See [Identities and Applications](/concepts/principal/) and [Model Your Application in Caracal](/guides/modeling-recipes/).
...
See [Troubleshoot by Symptom](/operations/troubleshooting/#diagnostic-bundle).
...
Start with [Inspect Diagnostics and Audit](/runtime-console/observability/) and [Debug Infrastructure Issues](/operations/debugging/).
...
See [Compare Editions](/enterprise/) or [book an enterprise call](https://cal.com/rawx18/caracal-enterprise-sales).
