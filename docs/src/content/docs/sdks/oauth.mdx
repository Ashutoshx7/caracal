...
The OAuth packages implement RFC 8693 token exchange. They exchange a long-lived ambient token, client secret, or delegated context for a short-lived per-resource mandate, cache the result by the full exchange context, and surface step-up challenges as typed errors where the language surface supports it. The main SDKs use these packages internally; install them directly only when building custom exchange logic or tools that call STS directly.
