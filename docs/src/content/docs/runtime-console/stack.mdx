...
Control runtime exposure is managed through the Console `control` menu, not by targeting a stack service directly from the shell.
...
| Console cannot connect after `up` | Check `caracal status --ready`, then confirm `CARACAL_API_URL` if you are not using localhost. |
