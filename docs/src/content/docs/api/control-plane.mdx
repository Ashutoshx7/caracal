...
| --- | --- | --- | --- | --- |
| `name` | string | Yes | None | 1+ characters |
| `org_id` | string | No | `"default"` | 1+ characters |
| `slug` | string | No | auto-derived | `^[a-z0-9-]+$`, globally unique |
| `dcr_enabled` | boolean | No | `false` | None |
| `pkce_required` | boolean | No | `true` | None |
| `login_flow` | string | No | `"default"` | None |
...
| `registration_method` | `"managed" \| "dcr"` | Yes | None |
| `credential_type` | `"token" \| "password" \| "public-key" \| "url" \| "public"` | No | `"public"` |
| `client_secret` | string | No | None | Hashed before storage; never returned |
| `traits` | string[] | No | `[]` |
...
**Constraints:** 1 to 256 entries; no duplicates; all versions must belong to the same zone; all policies must declare `package caracal.authz` and emit `data.caracal.authz.result`.
...
| `resource_id` | UUID | Yes | None |
| `scopes` | string[] | Yes | 1 to 64 scopes; each scope: `^[a-z0-9:_./-]+$`, ≤ 200 chars; must be a subset of the resource's scopes |
