...
#
# Downloads TUI binaries from the GitHub Release, verifies SHA256, and smoke-launches the host build headlessly.
...
validatePlat() {
  ...
  local file="caracal-tui-${plat}${ext}"
  local dir; dir="$(mktemp -d)"
  if ! runOrEcho curl -fsSL -o "$dir/$file" "$BASE/$file"; then
    logFinding "$AREA" "$file" "$plat" "github" "-" "$SEV_BLOCKER" "$STATUS_FAIL" "download failed" "curl $BASE/$file"
    return 0
  fi
  if ! runOrEcho curl -fsSL -o "$dir/SHA256SUMS" "$BASE/SHA256SUMS"; then
    logFinding "$AREA" "$file" "$plat" "github" "-" "$SEV_MAJOR" "$STATUS_FAIL" "SHA256SUMS missing" "curl $BASE/SHA256SUMS"
    rm -rf "$dir"; return 0
  fi
  if [[ "$DRY_RUN" != "1" ]]; then
  ...
  if [[ "$plat" == "$(hostPlatform)" && "$DRY_RUN" != "1" ]]; then
    chmod +x "$dir/$file"
    if timeout 8 "$dir/$file" --version 2>"$dir/err" | grep -q "$EXPECT"; then
      logFinding "$AREA" "$file" "$plat" "github" "-" "$SEV_INFO" "$STATUS_PASS" "--version returns $EXPECT" "./$file --version"
    else
      logFinding "$AREA" "$file" "$plat" "github" "-" "$SEV_MAJOR" "$STATUS_FAIL" "$(head -c 200 "$dir/err")" "./$file --version"
    fi
  else
