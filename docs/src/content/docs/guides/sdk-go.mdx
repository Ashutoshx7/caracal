...
Use the Console-generated `caracal.toml` profile for the first integration. `sdk.Connect()` reads `CARACAL_CONFIG`, then the default profile at `$XDG_CONFIG_HOME/caracal/caracal.toml` or `~/.config/caracal/caracal.toml`, then environment variables.
...
client := &sdk.Caracal{
    ...
    TokenSource: func(context.Context) (string, error) { return os.Getenv("MY_APP_TOKEN"), nil },
...
mux.Handle("/api/", client.ContextMiddleware(http.HandlerFunc(handler)))
