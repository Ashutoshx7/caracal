...
The fixture providers are only the local lab boundary. Production use must replace `_mock/` with real provider endpoints, real provider credentials from the platform secret manager, measured provider rate limits, and incident-owned rollback paths. Do not treat fixture response timing, fixture credentials, or fixture data volume as production evidence; use them only to exercise Caracal's real runtime, policy, Gateway, STS, and audit paths.
...
For production-shaped validation, replace each fixture URL with the provider endpoint approved for the target environment, store provider credentials outside the repository, and run the first-success validator against a real protected path. A production acceptance run should record Gateway success, STS decision evidence, provider response status, and Audit traceability for the same request ID.
