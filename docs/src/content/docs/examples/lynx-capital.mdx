...
The setup page also exposes an auto-setup button that runs the same automation
as the command-line script:

```bash
python setupCaracal.py
```

The script starts Caracal, creates the zone, creates the control API key,
registers every provider resource, writes `~/.config/caracal/caracal.toml`, and
copies `.env.example` to `.env` if needed. The generated config includes SDK
resource bindings:
...
All 14 external providers run as deterministic mock services in Docker. The
mock network joins `caracalData` so the real Caracal gateway can forward REST
...
`LYNX_MOCK_FAST=1` speeds up mock response latencies during development. The
`dev-*` provider credentials are accepted only by the mock third-party
providers. They are not Caracal credentials and should be replaced if you point
Lynx at real provider APIs.
...
configuration, and `POST /api/setup/caracal/auto` runs the Lynx Caracal setup
script from the setup page.
...
transport logic, and the Caracal boundary. Provider tests use the same mock
services and do not require live third-party credentials.
