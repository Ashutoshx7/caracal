...
Catch it to initiate the external step-up flow:
...
The current OAuth clients expose only the challenge ID. They do not expose `challenge_secret` and do not accept `challenge_response`, so they cannot complete the atomic step-up consumption retry by themselves. For a complete step-up flow, make the initial challenge-producing request and the retry directly against `POST {stsUrl}/oauth/2/token`.
...
The Python client validates successful STS responses before caching them. Cache keys include the session mandate, actor token, session and delegation context, scopes, TTL, and client authentication context. The default cache is in-memory and process-local.
