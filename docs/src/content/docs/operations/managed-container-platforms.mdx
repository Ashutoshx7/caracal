...
3. Copy `infra/containerPlatform/examples/azure.yaml`, set your endpoints, public origins, and provider wiring, and render:

   ```bash
   node infra/containerPlatform/render.mjs --config deployment.yaml --out ./rendered
   ```
...
4. Apply in order. Schema migrations must finish before any service revision rolls, because a release's migrations are expand-only against the version still serving:

   ```bash
   CARACAL_RESOURCE_GROUP=<group> bash infra/containerPlatform/scripts/deployAzure.sh ./rendered
   ```

5. Restore what the platform does not provide: subnet-level network restrictions, provider-native metrics and alert rules equivalent to the chart's, and backup and restore for Postgres and Redis.
