...
By default the installer writes to `~/.local/bin` and requires provenance verification. Override the install path with `--install-dir /path/to/bin` or `CARACAL_INSTALL_DIR=/path/to/bin`. Use `--no-verify-provenance` only when you explicitly need to skip provenance verification.
