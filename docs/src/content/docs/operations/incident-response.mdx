...
Self-service zone signing-key rotation is not packaged as a terminal interface/API operation in the OSS control plane. If signing material may have been exposed, perform the database-backed signing-key replacement through your deployment automation or incident runbook, then invalidate STS key caches and verify JWKS for every affected zone before restoring traffic.
