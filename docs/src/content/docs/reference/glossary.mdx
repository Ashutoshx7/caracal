...
A JWT with `"use": "ambient"` and a 60-minute TTL. Ambient mandates represent re-presentable session authority — they are issued for an execution session and presented back to the STS to derive narrower per-call mandates.

Ambient mandates are not accepted by the Gateway as upstream authorization credentials. They should not be accepted by production resources directly because they are not scoped to a specific resource or operation.
...
A record in the `delegation_edges` table representing one typed narrowing of authority from a source agent session to a target agent session. The edge carries source and target sessions, issuer and receiver applications, optional resource binding, scopes, expiry, and [caveats](#caveat). Delegation edges are immutable once created except for lifecycle status.
...
A JWT with `"use": "per_call"` and a 15-minute TTL. Per-call mandates are scoped to specific target resources and scopes. They are the preferred form for Gateway-mediated and connector-verified operations because they are short-lived, resource-bound, and rejected as `subject_token` inputs for further exchange.
