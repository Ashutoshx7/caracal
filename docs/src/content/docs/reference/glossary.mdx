...
A mode setting that controls how the Caracal terminal interface handles tool calls made through MCP (Model Context Protocol) servers. Two modes:
...
Session state is stored in the `sessions` table. Active sessions can be revoked through the `caracal session revoke` terminal interface command or the API.
