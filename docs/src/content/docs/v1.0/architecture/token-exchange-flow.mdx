...
slug: v1.0/architecture/token-exchange-flow
---
...
`caracal run` waits and retries once. SDK applications should follow their language SDK's interaction-required contract. See [Approvals](/v1.0/concepts/approvals/) for the canonical model.
...
For exact request fields and errors, use [Use STS Endpoint](/v1.0/api/sts/). Do not call STS internal policy or key-rotation routes; the API service owns those calls.
