---
title: Modeling Recipes
description: Decision-oriented recipes for mapping zones, applications, resources, and grants onto real deployments, from single-tenant apps to multi-customer platforms.
---

The [concept pages](/concepts/) define each noun, and [Provider Recipes](/guides/provider-recipes/) give concrete upstream auth setups. This page sits between them: it helps you decide **how to map your architecture onto Caracal's nouns** before you create anything.
...
Trade-off: lowest operational overhead. Add zones only when you need key, policy, or audit isolation — not before.
...
Decide on the strongest isolation a customer actually requires, then pick the least complex model that satisfies it. Do not encode customer identity into [scope names](/concepts/resource-grant/); keep it in the zone, the principal, or the policy input.

## High-sensitivity resource isolation
...
## App-only agents versus user-delegated agents
...
## Per-user OAuth grants versus a shared credential
...
- [Enterprise Runtime Patterns](/guides/enterprise-runtime-patterns/)
