...
Use `CARACAL_MODE=runtime` for runtime deployments and `CARACAL_MODE=dev` for local development. Runtime mode rejects developer-only escape hatches such as insecure STS or Gateway flags; configure Gateway TLS directly with certificate files or terminate TLS at a trusted ingress.
