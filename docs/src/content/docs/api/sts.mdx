...
Exchanges a session mandate (or acts as an application credential) for a short-lived resource mandate. The `resource` parameter is repeatable — include it multiple times to request a single mandate covering multiple resources.
...
| `subject_token` | string | Cond. | Session mandate from a prior token exchange; omit for pure application-credential exchanges |
...
| `ttl_seconds` | integer | No | Requested mandate lifetime in seconds; clamped to ≤ 900 for resource, ≤ 3600 for session |
...
3. **Session mandate validation** — If `subject_token` present: verifies ES256 signature against zone JWKS, checks issuer, audience, `zone_id` claim, and that `use` equals `"session"`. Verifies session record is active.
4. **Actor token validation** — If `actor_token` present: same JWT checks as the session mandate. Verifies actor and subject are distinct principals.
...
| `forward_caracal_identity` | boolean | Present only when provider config explicitly allows Gateway to forward `X-Caracal-Identity`; default false |
...
| `invalid_token` | 401 | `subject_token` or `actor_token` signature invalid, expired, or zone mismatch; `use` claim not `"session"`; subject and actor are the same principal |
