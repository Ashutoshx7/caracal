...
const sections: Record<string, string[]> = {
  ...
  'SDKs': ['sdks', 'sdks/typescript', 'sdks/python', 'sdks/go', 'sdks/identity', 'sdks/oauth', 'sdks/revocation', 'sdks/admin', 'sdks/transport-mcp', 'sdks/transport-a2a', 'sdks/connectors'],
  ...
  'Reference': ['reference', 'reference/glossary', 'reference/errors', 'reference/configuration', 'reference/config-precedence', 'reference/defaults-and-limits', 'reference/compatibility'],
...
export async function GET() {
  ...
  const lines: string[] = [
    ...
    '',
    'The runtime includes API (port 3000), STS (port 8080), Gateway (port 8081), Audit (port 9090), Coordinator (port 4000), and optional Control (port 8087). Runtime lifecycle uses the top-level caracal shell; product management uses Console, Admin SDK, or Control API.',
    '',
  ]
