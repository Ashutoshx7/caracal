---
title: Integrate the Python SDK
description: Install caracalai-sdk, use async context managers to spawn and delegate agent sessions, wire ASGI middleware, and make outbound calls with auto-injected headers.
pageType: workflow
---

The Python SDK uses async context managers for session lifecycle. `Caracal.connect()` reads the generated Caracal profile first, then falls back to environment variables. `spawn()` and `delegate()` are async context managers that open and close sessions automatically. `transport()` returns an `httpx.AsyncClient` with Caracal access headers injected on every request.

## Prerequisites

- Python 3.14 or later.
- A registered application in a Caracal zone with a `client_secret`.
- The Caracal stack running with the Coordinator reachable.

## Install

```bash
pip install caracalai-sdk
```

## Configuration

Use the Console-generated `caracal.toml` profile for the first integration. `Caracal.connect()` reads `config_path` when passed, then `CARACAL_CONFIG`, then the default profile at `$XDG_CONFIG_HOME/caracal/caracal.toml` or `~/.config/caracal/caracal.toml`, then environment variables.

For platform-managed workloads, set these environment variables instead of mounting a profile:

| Variable | Required | Description |
| -------- | -------- | ----------- |
| `CARACAL_COORDINATOR_URL` | Yes | Base URL of the Coordinator service (e.g., `http://localhost:4000`) |
| `CARACAL_ZONE_ID` | Yes | Zone identifier |
| `CARACAL_APPLICATION_ID` | Yes | Registered application ID |
| `CARACAL_APP_CLIENT_SECRET_FILE` | Preferred | Secret-file path used to refresh session authority through STS |
| `CARACAL_APP_CLIENT_SECRET` | Local development | Application secret used to refresh session authority through STS |
| `CARACAL_STS_URL` | Client-secret mode | STS base URL |
| `CARACAL_APP_RESOURCES` | Client-secret mode if `CARACAL_RESOURCES` is unset | Comma-separated resource IDs for token exchange |
| `CARACAL_GATEWAY_URL` | No | Gateway base URL for resource-routed calls |
| `CARACAL_RESOURCES` | No | Comma-separated bindings: `resourceId=upstreamPrefix,other=prefix` |
| `CARACAL_RESOURCES_FILE` | No | JSON resource-binding file |

## Construct the client

```python
from caracalai_sdk import Caracal

# Reads the generated profile first, then falls back to CARACAL_* env vars.
caracal = Caracal.connect()
```

To configure programmatically:

```python
import os
from caracalai_sdk import Caracal, CaracalConfig
from caracalai_sdk.advanced import CoordinatorClient, AgentKind, ResourceBinding

caracal = Caracal(CaracalConfig(
    coordinator=CoordinatorClient(base_url='http://coordinator:4000'),
    zone_id='my-zone',
    application_id='my-app',
    token_source=lambda: os.environ['MY_APP_TOKEN'],
    gateway_url='http://gateway:8081',
    resources=[
        ResourceBinding(
            resource_id='resource://inventory',
            upstream_prefix='http://inventory:8080',
        ),
    ],
    default_kind=AgentKind.INSTANCE,
))
```

## Spawn an agent session

`spawn()` is an async context manager. It opens an agent session on the Coordinator at entry, binds it to the current async task via Python context vars, and closes the session at exit.

```python
async def main():
    async with caracal.spawn() as ctx:
        print('agent session:', ctx.agent_session_id)
        # All awaits inside this block share the agent session.
```

Options:

```python
from caracalai_sdk.advanced import AgentKind

async with caracal.spawn(
    kind=AgentKind.EPHEMERAL,
    ttl_seconds=600,
    metadata={'purpose': 'invoice-batch'},
) as ctx:
    ...
```

## Access session context

Inside a `spawn()` block, `caracal.current()` returns the live context:

```python
async def process_invoice(invoice_id: str):
    ctx = caracal.current()
    # ctx.agent_session_id, ctx.zone_id, ctx.delegation_edge_id, ctx.hop
```

## Delegate authority

`delegate()` creates a delegation edge from the current session to a target session. All calls inside the block carry the delegated authority.

```python
from caracalai_sdk import DelegationConstraints

async with caracal.spawn() as source:
    async with caracal.delegate(
        to=target_agent_session_id,
        to_application_id='payments-service',
        scopes=['payment:submit', 'payment:read'],
        constraints=DelegationConstraints(resources=['resource://payments'], max_depth=2),
        ttl_seconds=300,
    ) as delegated:
        print('delegation edge:', delegated.delegation_edge_id)
        # Outbound calls carry this edge in W3C Baggage
```

## Make outbound calls

`caracal.transport()` returns an `httpx.AsyncClient`. It reads the current context from Python context vars and injects:

- `Authorization: Bearer <access_token>`
- `traceparent: 00-{traceId}-{spanId}-01`
- `baggage: caracal.agent_session=…;caracal.delegation_edge=…;caracal.hop=N`

```python
async with caracal.spawn():
    async with caracal.transport() as client:
        response = await client.get('http://inventory:8080/items')
        items = response.json()
```

## Receive inbound context

After a request has been verified by Gateway or a trusted adapter, bind context from inbound HTTP headers to propagate the calling agent's identity into the current task:

```python
async with caracal.bind_from_headers(request.headers):
    ctx = caracal.current()
    # ctx is populated from the inbound Authorization and baggage headers
```

## ASGI middleware

For FastAPI and Starlette, attach the ASGI middleware to bind already-verified inbound context automatically on every request:

```python
from fastapi import FastAPI

app = FastAPI()
app.add_middleware(caracal.context_middleware())

@app.get('/items')
async def list_items():
    ctx = caracal.current()
    return {'agent': ctx.agent_session_id if ctx else None}
```

The SDK ASGI context middleware is not a verifier. It does not check JWT signatures, audience, scopes, token use, or revocation. Use Gateway for the default protected-resource path. If the service must verify directly, build the adapter around `caracalai_transport_mcp.authenticate()` and a production revocation store.

## Complete example

```python
import asyncio
from caracalai_sdk import Caracal
from caracalai_sdk.advanced import AgentKind

caracal = Caracal.connect()

async def run_batch(invoice_ids: list[str]):
    async with caracal.spawn(kind=AgentKind.EPHEMERAL, ttl_seconds=120) as ctx:
        print(f'processing {len(invoice_ids)} invoices in session {ctx.agent_session_id}')

        async with caracal.transport() as client:
            for invoice_id in invoice_ids:
                response = await client.get(f'http://invoices:8080/invoices/{invoice_id}')
                response.raise_for_status()
                print(response.json())

asyncio.run(run_batch(['inv-001', 'inv-002', 'inv-003']))
```

## What to read next

- [Protect a FastMCP App](./protect-fastmcp/): gate FastMCP tool calls with mandate auth
- [Implement Multi-Agent Delegation](./delegation/): deeper delegation graph patterns
- [Protect an MCP Server](./protect-mcp/): low-level mandate verification for custom servers
