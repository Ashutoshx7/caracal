...
infra/secrets/secret-init.sh
```

`infra/docker/.env.example` documents every required and optional variable. Copy it to `infra/docker/.env` and set your values. `secret-init.sh` generates the Docker secret files in `infra/secrets/files/` (gitignored) that the compose stack mounts for postgres, Redis, and the application services. Run it once before the first `caracal up`; it is idempotent and safe to re-run.
