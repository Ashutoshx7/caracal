...
All customers share one zone policy set, but policy decisions are per-request and subject-aware. The customer's `sub` and claims, the agent's capabilities, and the delegation edge are all in the policy input, so one policy set can allow different customers different authority at the same time.
