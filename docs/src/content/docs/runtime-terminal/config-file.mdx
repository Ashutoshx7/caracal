...
The terminal management interface resolves the config path in this order, stopping at the first match:
...
These environment variables are read by the terminal management interface. Most of them override what is in `caracal.toml` or the defaults applied in development mode:
